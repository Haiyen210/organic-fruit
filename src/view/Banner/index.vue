...
export default {
  ...
  mounted() {
      axios
        .get("http://localhost:8080/Oganic_Fruit/rest/bannerService/getListBanner")
        .then((res) => {
  ...
  methods: {
    ...
    onDelete(item) {
      if (confirm("Bạn có chắc muốn xóa Banner số " + item.id)) {
        ...
        axios
          .delete(
            `http://localhost:8080/Oganic_Fruit/rest/bannerService/deleteBanner/${item.id}`
          )
          .then((response) => {
